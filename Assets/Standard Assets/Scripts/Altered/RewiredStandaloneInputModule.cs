...
namespace Rewired.Integration.UnityUI {
	...
	public class RewiredStandaloneInputModule : PointerInputModule {
		...
		public static RewiredStandaloneInputModule instance;
		...
		public override void UpdateModule ()
		{
			if (previousSelected != null)
			{
				...
				Selectable previousSelectedSelectable = previousSelected.GetComponent<Selectable>();
				colors = previousSelectedSelectable.colors;
				colors.colorMultiplier = 1;
				previousSelectedSelectable.colors = colors;
			}
			...
			DealWithInvalidSelection ();
			if (eventSystem.currentSelectedGameObject != previousSelected)
			{
				colorMultiplier.JumpToStart();
				// if (eventSystem.currentSelectedGameObject.GetComponent<Scrollbar>() == null)// && Global.GetGameObjectWithComponentInParents(eventSystem.currentSelectedGameObject.transform, "Dropdown") == null)
				// 	ScrollToSelected ();
			}
			else
				ColorSelected ();
		...
		public virtual void DealWithInvalidSelection ()
		{
			_Selectable navigatable = eventSystem.currentSelectedGameObject.GetComponent<_Selectable>();
			if (navigatable == null)
			{
				eventSystem.SetSelectedGameObject(previousSelected, GetBaseEventData());
				if (previousSelected != null)
					navigatable = previousSelected.GetComponent<_Selectable>();
			}
		}
		
		public virtual void ColorSelected ()
		{
			ColorBlock colors = new ColorBlock();
			Selectable selected = eventSystem.currentSelectedGameObject.GetComponent<Selectable>();
			colors = selected.colors;
			colors.colorMultiplier = colorMultiplier.GetValue();
			selected.colors = colors;
		}
		
		// public virtual void ScrollToSelected ()
		// {
		// 	GameObject scrollRectGO = GameObjectExtensions.FindWithComponentInParents((Transform) eventSystem.currentSelectedGameObject.GetComponent<RectTransform>(), "ScrollRect");
		// 	if (scrollRectGO != null)
		// 	{
		// 		ScrollRect scrollRect = scrollRectGO.GetComponent<ScrollRect>();
		// 		RectTransform viewport = (RectTransform) scrollRect.transform.GetChild(0);
		// 		Vector3[] viewportCorners = new Vector3[4];
		// 		viewport.GetWorldCorners(viewportCorners);
		// 		Rect viewportRect = Rect.MinMaxRect(viewportCorners[0].x, viewportCorners[0].y, viewportCorners[1].x, viewportCorners[1].y);
		// 		RectTransform content = (RectTransform) viewport.GetChild(0);
		// 		RectTransform selectedContent = eventSystem.currentSelectedGameObject.GetComponent<RectTransform>();
		// 		Vector3[] selectedContentCorners = new Vector3[4];
		// 		selectedContent.GetWorldCorners(selectedContentCorners);
		// 		float scrollToY = Mathf.Abs(selectedContent.localPosition.y) - selectedContent.sizeDelta.y / 2;
		// 		float scrollValue = 1 - 1 / ((content.sizeDelta.y) / scrollToY);
		// 		bool selectedContentTopIsVisible = viewportCorners[0].y < selectedContentCorners[0].y;
		// 		bool selectedContentBottomIsVisible = viewportCorners[2].y > selectedContentCorners[2].y;
		// 		if (!selectedContentTopIsVisible || !selectedContentBottomIsVisible)
		...
		public override void ActivateModule ()
		{
			...
			GameObject currentSelectedGo = eventSystem.currentSelectedGameObject;
			if (currentSelectedGo == null)
				currentSelectedGo = eventSystem.firstSelectedGameObject;
			
			eventSystem.SetSelectedGameObject(currentSelectedGo, GetBaseEventData());
		}
		...
		protected bool SendMoveEventToSelectedObject() {
			...
			bool buttonJustPressed = CheckButtonOrKeyMovement();
		...
		private bool CheckButtonOrKeyMovement() {
			bool allow = false;
